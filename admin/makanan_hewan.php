...
if ($action == "update") {
    ...
    $query = "SELECT * FROM makanan_hewan WHERE id_Makanan = '$id_Makanan'";
...
if ($action == "delete") {
    $id_Makanan = $_GET['id_Makanan'];
    $query = "CALL DEL_MAKANAN_HWN('$id_Makanan')";
    $result = mysqli_query($conn, $query);
    if ($result) {
        header("Location: makanan_hewan.php");
    } else {
...
if (isset($_POST['insert'])) {
    ...
    $QTY = $_POST['QTY'];
    if ($action == "update") {
        $query = "CALL UPD_MAKANAN_HWN('$id_Makanan','$jenis_Makanan','$harga_Makanan','$QTY')";
        $result = mysqli_query($conn, $query);
        if ($result) {
            header("Location: makanan_hewan.php");
        } else {
        ...
        $query = "CALL ADD_MAKANAN_HWN('$id_Makanan','$jenis_Makanan','$harga_Makanan','$QTY')";
        $result = mysqli_query($conn, $query);
        if ($result) {
            header("Location: makanan_hewan.php");
        } else {
        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-secondary navbar-dark">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="makanan_hewan.php" class="nav-item nav-link active"><i class="fas fa-bone me-2"></i>MAKANAN</a>
                ...
                <div class="row g-4">
                    <div>
                        <div class="bg-secondary rounded h-100 p-4">
                            <h6 class="mb-4">Makanan</h6>
                            <form action="" method="POST">
                                <div class="form-floating mb-3">
                                    <input name="id_Makanan" type="text" class="form-control" id="floatingInput" placeholder="Id Layanan" value="<?PHP echo $id_Makanan ?>">
                                    <label for="floatingInput">Id Makanan</label>
                                </div>
                                <div class="form-floating mb-3">
                                    <select value="<?PHP echo $jenis_Makanan ?>" name="jenis_Makanan" class="form-select" id="floatingSelect" aria-label="Floating label select example">
                                        <option selected>Jenis Makanan</option>
                                        <?PHP
                                        $jenis_Makanan_Hewan = array("Whiskas", "Royal Canin", "Pro Plan", "Science Diet", "Jerhigh");
                                        foreach ($jenis_Makanan_Hewan as $value) {
                                        ?>
                                            <option value="<?php echo $value ?>" <?php if ($jenis_Makanan === $value) {
                                ...
                                <div class="form-floating mb-3">
                                    <input name="harga_Makanan" type="number" class="form-control" id="floatingPassword" placeholder="Nama Layanan" value="<?PHP echo $harga_Makanan ?>">
                                    <label for="floatingPassword">Harga Makanan</label>
                                </div>
                                <div class="form-floating mb-3">
                                    <input value="<?PHP echo $QTY ?>" name="QTY" type="number" class="form-control" id="floatingPassword" placeholder="Harga Layanan">
                                    <label for="floatingPassword">Quatity</label>
                                </div>
                                <div class="form-floating mb-3 ">
                    ...
                    <div>
                        <div class="bg-secondary rounded h-100 p-4">
                            <h6 class="mb-4">TABEL MAKANAN</h6>
                                ...
                                <table class="table" id="table_id">
                                    ...
                                    <tbody>
                                        ...
                                        while ($row = mysqli_fetch_array($result)) {
                                            ...
                                            echo "<td>" . $row['QTY'] . "</td>";
                                            echo '<td>';
                                            echo "<a href='makanan_hewan.php?action=update&id_Makanan=" . $row['ID_MAKANAN'] . "' class='btn btn-primary m-1 btn-edit'>Edit</a>";
                                            echo "<a href='makanan_hewan.php?action=delete&id_Makanan=" . $row['ID_MAKANAN'] . "' onclick='return confirm(\"Apakah anda yakin ingin menghapus data ?\")' class='btn btn-danger m-1'>Delete</a>";
                                            echo '</td>';
                                            echo "</tr>";
                                        }
                                        ?>
